...
client.on("guildMemberAdd", async member => {
  ...
  if (!otorol) return;
  log2.send(
    `:mega:**☣ Demon Light = * *☣* *  !**\n> **Hoşgeldin,** <@${member.user.id}>   ● ● ● ● ● ●  \n> \n> **Sunucuya Kayıt Olmak İçin Confirmed Odalarından Birine Geçebilirsiniz** \n> <@&644112189799530506> **Rolünde Olan Yetkilileri Etiketleyip Teyit Odasında Bekleyiniz** `
  );
  
    const id = "697926933828075541";
  ...
  setInterval(() => {
  client.channels
    .get("697926933828075541")
    .send(
      "<@&644112197492015146> Kayıt İçin Confirmed Odalarına Geçebilirsiniz."
    );
}, 1 * 60 * 60 * 1000);
...
client.on("ready", () => {
  client.channels.get("697926139540406345").join();
...
client.on("ready", () => {
  ...
  console.log("❂ DEMON LİGHT giriş yaptı");
  client.user.setActivity("❂ DEMON LİGHT = * *❂* * DİKKAT ET TEPENDEYİM #TagdaKal", {
    type: "GAME"
  });
});